...
API_URL = "http://127.0.0.1:5000/api/verificar-premium"  # Substitua pelo URL real em produção

def verificar_status_premium(token):
    try:
        headers = {"Authorization": f"Bearer {token}"}
        resposta = requests.get(API_URL, headers=headers, timeout=5)
        if resposta.status_code == 200 and resposta.json()["premium"]:
            return True
        return False
...
token = simpledialog.askstring("Autenticação", "Digite seu token de acesso (obtido no site):")
if not token or not verificar_status_premium(token):
    messagebox.showerror("Erro", "Acesso negado. Você precisa de um plano premium para usar este programa.")
    sys.exit(1)
...
# Iniciar o programa
root.deiconify()  # Mostra a janela após a autenticação
